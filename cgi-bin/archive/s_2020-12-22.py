...
def form_to_qs(form, element):
    ...
    print('</pre>')
    print(element)
    read_file = open ("./input/"+element+".json", mode='r', encoding="utf-8")
    for line in read_file.readlines():
        ...
        print('power:', float(data['importance'])*float(data['probability']))
    print('''
    <br>
...
def qs_to_file(form, element):
    ...
    if 'name' in form  and  'actions' in form and 'importance' in form  and 'probability' in form :
        ...
        #print('\n element:')
        #print(element)
        with open("./input/"+element+".json", "w", encoding="utf-8") as write_file:
            pass
        with open("./input/"+element+".json", "a", encoding="utf-8") as write_file:
...
def swot():
    """"Элементы и параметры SWOT-анализа"""
    print(swot.__doc__)
    swot_matrix = {}
...
def parameters_of_element(element):
    ...
    power_list = list()
    with open("./results/"+element+".json", "w", encoding="utf-8") as write_file:
        pass
    read_file = open ("./input/"+element+".json", mode='r', encoding="utf-8")
    for line in read_file.readlines():
        #print(line, end="")
        ...
        print(' name:', parameters_dictionary['name'],  ' actions:',parameters_dictionary['actions'],  ' importance:', parameters_dictionary['importance'],  'probability:', parameters_dictionary['probability'],  ' power:', parameters_dictionary['power'] )
        json_str = json.dumps(parameters_dictionary,  ensure_ascii=False, sort_keys=False)
        with open("./results/"+element+".json", "a", encoding="utf-8") as write_file:
            write_file.write(json_str)
            write_file.write("\n")
    read_file.close()
